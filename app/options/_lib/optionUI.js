...
import {
    ...
    appendFavoriteOption,
    getOptionChildsFromId,
    getOptionParentFromId,
    moveFavoriteOption,
} from "./optionLib.js"
        ...
        categories: [
            {
                ...
                description: {
                    attention:
                        "【デバッグモード】開発者向けの機能です。不具合が発生する可能性がありますのでご注意ください。",
                },
            },
            {
                title: "Narou Tweaker",
...
export function getOptionElement(option, mode) {
    ...
    if (title || description) {
            ...
            if (mode === "favorite") {
                if (!elm.find(".contents-item--heading").length) {
                    elm.find(".contents-option-head").append(
                        `<div class="contents-item--heading"><a href="/options/${page}/index.html?id=${id}&focus=1" target="_self">${title}</a></div>`
                    )
                } else {
                ...
                */
                ...
                if (!elm.find(".contents-item--heading").length) {
                    elm.find(".contents-option-head").append(
                        `<div class="contents-item--heading">${title}</div>`
                    )
                } else {
                    elm.find(".contents-item--heading").empty()
        ...
        if (description) {
            if (description.text) {
                descriptionText.push(
                    `<div class="contents-item--description-item">${description.text}</div>`
                )
            }
            if (description.small) {
                descriptionText.push(
                    `<div class="contents-item--description-item description-small">${description.small}</div>`
                )
            }
            if (description.attention) {
                descriptionText.push(
                    `<div class="contents-item--description-item description-attention">${description.attention}</div>`
                )
            }
        }
        ...
        if (descriptionText.length > 0) {
            if (!elm.find(".contents-item--description").length) {
                elm.find(".contents-option-head").append(
                    `<div class="contents-item--description">${descriptionText.join("")}</div>`
                )
            } else {
    ...
    if (buttons) {
        ...
        if (buttons.favorite) {
            /* favorite buttons */
            var buttonOuterElm = $(
                `<div class="contents-item--button-item contents-item--button-favorite"></div>`
            )
            ...
            if (mode === "favorite") {
                ...
                if (!hasParent) {
                    ...
                    buttonElm.append(
                        `<div class="contents-item--button-item contents-item--button-separator"></div>`
                    )
                }
    ...
    if (requirement) {
        ...
        var hsDataType = new Array(len).fill(undefined).map((v, k) => {
