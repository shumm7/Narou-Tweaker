...
function exceptionProcess_local(oldDict, newDict) {
    ...
    if (oldDict.novelCustomHeader === true) {
        console.log(
            `Converted value: { novelCustomHeader: true } -> { novelCustomHeaderType: "2" } `
        )
        newDict.novelCustomHeaderType = "2"
    } else if (oldDict.novelCustomHeader === false) {
        console.log(
            `Converted value: { novelCustomHeader: false } -> { novelCustomHeaderType: "1" } `
        )
    ...
    Array(
        "workspaceCustomMenu_Left",
        "workspaceCustomMenu_Middle",
        "workspaceCustomMenu_Right"
...
function checkOptionValue(key, value) {
    ...
    if (typeof defaultOption[key] === typeof value && value !== undefined) {
        if (key === "novelCustomHeaderMode" || key === "workspaceCustomHeaderMode") {
            ...
            key === "correctionNumberShort" ||
            key === "correctionNumberLong" ||
            key === "correctionNumberSymbol"
        ) {
